      ...
      <div className="mx-auto max-w-[1440px] space-y-8">
          ...
          {editable ? (
            <form action="/api/profile/username" method="post" className="flex w-full max-w-md items-center gap-2">
              <input
                ...
                maxLength={24}
                required
                className={`${jetMono.className} w-full border-[3px] border-[#0D0D0D] bg-[#212121] px-3 py-2 text-sm text-white`}
              />
              <button
                type="submit"
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {primaryStats.map((stat) => (
            <article key={stat.label} className="border-[4px] border-[#0D0D0D] bg-[#212121] px-4 py-6">
              <p className={`${oswald.className} text-5xl leading-none`} style={{ color: stat.color ?? "#F5F5F0" }}>
                {stat.value}
              </p>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {secondaryStats.map((stat) => (
            <article key={stat.label} className="border-[4px] border-[#0D0D0D] bg-[#212121] px-4 py-5">
